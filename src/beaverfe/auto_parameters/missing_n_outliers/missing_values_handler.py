...
class MissingValuesHandlerParameterSelector:
    ...
    def _evaluate_strategy(
        ...
        return float(evaluate_model(X, y, model, scoring, cv, groups, transformer))

    def _build_result(
        self, strategies: dict[str, str | None], knn_params: dict[str, int]
    ) -> dict[str, object]:
        transformer = MissingValuesHandler(
    ...
    def _is_column_important(self, X_col, y) -> bool:
        if X_col.dtype.kind in "bifc" and y.dtype.kind in "bifc":
            return bool(abs(X_col.corr(y)) > 0.3)
